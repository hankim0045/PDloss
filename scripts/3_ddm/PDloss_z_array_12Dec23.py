#####################################################################################
# PDloss z model (Run on 12 Dec 2023; n = 48; samples = 30000)
...
## Specify model
db_filename = (f'PDloss_z_12Dec23_{iteration}.db')
model_filename = (f'PDloss_z_12Dec23_{iteration}')

v_reg = {'model': 'v ~ 1 + stim', 'link_func': lambda x: x}  
z_reg = {'model': 'z ~ 1 + condition', 'link_func': lambda x: x}
reg_descr = [v_reg, z_reg]

z_model = hddm.models.HDDMRegressor(finaldata, reg_descr,
                                       bias=True, include='p_outlier',
                                       group_only_regressors=False)

z_model.find_starting_values()
z_model.sample(samples, burn=burnnum, thin=2, dbname=db_filename, db='pickle') 
z_model.save(model_filename)


## Compute DIC (deviance information criterion)
print("Model DIC: %f" % z_model.dic)


## Compute DICc (deviance information criterion, corrected)
DICc = z_model.dic_info['deviance'] + z_model.dic_info['pD'] + z_model.dic_info['pD']
